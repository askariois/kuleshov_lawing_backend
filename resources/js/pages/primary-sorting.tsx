...
export default function PrimarySorting() {
   ...
   const onImage = (status: string) => {
      router.post(`/primary-sorting/${img.id}/sort`, {
         status,
         ...
         onSuccess: () => {
            ...
            router.visit(`/primary-sorting/${projectId}?page=${nextPage}`, {
               preserveState: true,
               preserveScroll: true,
            });
         },
   ...
   return (
      <AppLayout>
         <Header title="Первичная сортировка" subtitle={`Всего: ${images.to} / ${images.total}`}>
         </Header>
         {images.data.map((img) => {
            const [name, ext] = img.name.split(/\.(?=[^\.]+$)/); // делим по последней точке

            return (<div className="flex  flex-row gap-6">
               <div className="basis-1/2 w-[520px] h-[520px] flex justify-center items-center bg-[#00000050] backdrop-blur-sm z-40 rounded-2xl">
         ...
         <div className="mt-6 ">
            <div className="flex justify-between gap-1.5 mb-1.5">
               ...
               <Button variant="secondary" size={'full'} className='text-[18px] font-semibold'>
                  Авторское
               </Button>
               <Button variant="secondary" size={'full'} className='text-[18px] font-semibold'>
                  Уточнить у клиента
         ...
         <div className='w-full mt-9 flex'>
            <TextLink href={`/images/${id}`} variant="primary" size={'lg'} className='ml-auto'>
               Завершить обработку
            </TextLink>
         </div>
      </AppLayout >
